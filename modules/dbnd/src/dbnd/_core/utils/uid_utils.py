...
def get_task_def_uid(dag_id, task_id, code_hash):
    return uuid.uuid5(NAMESPACE_DBND_TASK_DEF, f"{dag_id}.{task_id}.{code_hash}")


def get_task_run_uid(run_uid, dag_id, task_id):
    return uuid.uuid5(run_uid, f"{dag_id}.{task_id}")


def get_task_run_attempt_uid(run_uid, dag_id, task_id, try_number):
    return uuid.uuid5(run_uid, f"{dag_id}.{task_id}:{try_number}")
...
def get_job_run_uid(airflow_instance_uid, dag_id, execution_date):
    ...
    if airflow_instance_uid is None:
        return uuid.uuid5(NAMESPACE_DBND_RUN, f"{dag_id}:{execution_date}")
    else:
        return uuid.uuid5(
            NAMESPACE_DBND_RUN,
            f"{airflow_instance_uid}:{dag_id}:{execution_date}",
        )
...
def get_job_uid(airflow_server_info_uid, dag_id):
    ...
    if airflow_server_info_uid:
        return uuid.uuid5(NAMESPACE_DBND_JOB, f"{airflow_server_info_uid}:{dag_id}")
    else:
...
def get_dataset_op_uid(dataset_uid, task_run_attempt_id, operation_type):
    return uuid.uuid5(dataset_uid, f"{task_run_attempt_id}.{operation_type}")
