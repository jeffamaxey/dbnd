...
class TaskMetaclass(abc.ABCMeta):
    ...
    def _build_task_obj(self, **kwargs):
        # called from TaskFactory to create object
        return super(TaskMetaclass, self).__call__(**kwargs)
    ...
    def __call__(self, *args, **kwargs):
        ...
        if (
            is_in_airflow_dag_build_context()
            and TaskEssence.is_task_cls(self)
            and not _dbnd_disable_airflow_inplace
            and not getattr(self, "_dbnd_decorated_task", False)
        ):
            kwargs = kwargs.copy()
            kwargs["_dbnd_disable_airflow_inplace"] = True
            return build_task_at_airflow_dag_context(
                task_cls=self, call_args=args, call_kwargs=kwargs
            )
        ...
        with config(
            ...
            task_object = factory.build_task_object(self)
