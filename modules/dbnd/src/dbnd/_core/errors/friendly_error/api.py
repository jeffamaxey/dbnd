...
def api_connection_refused(connection_details, ex):
    return DatabandConnectionException(
        f"Connection Error: Could not connect to databand api server on host: {connection_details} ",
        show_exc_info=False,
...
def run_uid_not_found_in_db(run_uid):
    return DatabandConnectionException(
        f"Could not find run uid '{run_uid}' in current database, are you using same DB for submission and execution?",
        show_exc_info=False,
