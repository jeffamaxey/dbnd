...
def local_engine_not_accept_remote_jobs(env, to_cluster):
    return DatabandConfigError(
        ...
        help_msg=f"Check your execution configuration at {env}",
    )


def kubernetes_with_non_compatible_engine(task_engine):
    return DatabandConfigError(
        f"Config env.task_executor_type expected to be airflow_kubernetes for {task_engine.task_name}"
    )
...
def kubernetes_image_not_found(image_name, message, long_msg):
    return KubernetesImageNotFoundError(
        f"Failed to start Kubernetes pod because the configured image ({image_name}) could not be pulled by Kubernetes: {message}",
...
def kubernetes_running_pod_fails_on_condition(condition, pod_name):
    return KubernetesRunningPodConditionFailure(
        f"Pod {pod_name} is failing at Running phase: {condition}",
...
def kubernetes_pod_config_error(kub_message):
    ...
    return KubernetesPodConfigFailure(
        f"Failed to start Kubernetes pod because of a configuration error: {kub_message}",
        help_msg=help_msg,
    )
...
def kubernetes_pod_unschedulable(kub_message, extra_help=None):
    ...
    return DatabandConfigError(
        f"Failed to start Kubernetes pod because it couldn't be scheduled. Reason: {kub_message}",
        help_msg=help_msg,
    )
...
def failed_to_connect_to_cluster(in_cluster_value, exc):
    return DatabandConfigError(
        f"Could not connect to kubernetes cluster! Exception: {exc}",
        help_msg=f"""in-cluster is set to '{in_cluster_value}'. Are you running {"inside" if in_cluster_value else "outside"} cluster?""",
    )
